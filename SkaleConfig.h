...
static constexpr size_t SHA3_HASHLEN = 32;

static constexpr size_t PARTIAL_SHA3_HASHLEN = 8;
...
// typedef array<uint8_t, SHA3_HASHLEN> sha3_hash;
class sha3_hash {
    typedef std::array<uint8_t, SHA3_HASHLEN> base;
    base arr;

public:
    using value_type = base::value_type;
    using size_tye = base::size_type;
    using difference_type = base::difference_type;
    using const_reference = base::reference;
...
public:
    ...
    sha3_hash(const uint8_t *arg) {
        for (size_t i = 0; i < SHA3_HASHLEN; ++i)
            this->arr[i] = arg[i];
    }

    sha3_hash(const sha3_hash &arg) : arr(arg.arr) {
        int x = 3;
        x = x + 1;
    }
...
private:
    sha3_hash &operator=(const sha3_hash &arg) {
        arr = arg.arr;
        return *this;
    }
};

typedef array<uint8_t, PARTIAL_SHA3_HASHLEN> partial_sha3_hash;
...
typedef map<schain_index, ptr<sha3_hash> > proposed_block_hashes;
...
inline void printHash(ptr<sha3_hash> hash) {
    for (size_t i = 0; i < SHA3_HASHLEN; i++) {
...
inline void printPartialHash(ptr<partial_sha3_hash> hash) {
    for (size_t i = 0; i < PARTIAL_SHA3_HASHLEN; i++) {
        cerr << to_string((*hash)[i]);
    }
}

inline void validateHash(ptr<sha3_hash> hash) {
    for (size_t i = 0; i < SHA3_HASHLEN; i++) {
        if ((*hash)[i] != 0) {
            return;
        }
...
std::string to_string(const sha3_hash &hash);
