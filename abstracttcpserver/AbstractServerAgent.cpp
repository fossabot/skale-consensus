...
void AbstractServerAgent::acceptTCPConnectionsLoop() {
    ...
    try {
        ...
        while (!getSchain()->getNode()->isExitRequested()) {
            ...
            if (newConnection < 0) {
                BOOST_THROW_EXCEPTION(NetworkProtocolException("accept failed", __CLASS_NAME__));
            }
    ...
    if (messageCount != 0) {
        try {
            getSchain()->getIo()->readBytes(_connectionEnvelope_,
                                             ...
                                             msg_len((uint64_t) partialHashesList->getTransactionCount() * PARTIAL_SHA3_HASHLEN));
