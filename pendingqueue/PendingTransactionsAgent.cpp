...
PendingTransactionsAgent::PendingTransactionsAgent( Schain& ref_sChain )
    ...
    string value;
    if (getSchain()->getNode()->getBlocksDB()->Get(options, "COUNT", &value).ok()) {
        committedTransactionCounter = stoul(value);
    }


    leveldb::Iterator* it = getNode()->getCommittedTransactionsDB()->NewIterator(leveldb::ReadOptions());
    for (it->SeekToFirst(); it->Valid(); it->Next()) {
        auto s = make_shared<partial_sha3_hash>();
        std::copy_n((uint8_t*)it->key().data(), PARTIAL_SHA3_HASHLEN, s->begin());
        addToCommitted(s);
        committedTransactionCounter++;
        if (committedTransactionCounter > getNode()->getCommittedTransactionsHistory())
            break;
    }
    delete it;
}

void PendingTransactionsAgent::addToCommitted(ptr<partial_sha3_hash> &s)  {
...

    {
        ...
        while (iterator != pendingTransactions.cend()) {
            if (isCommitted(iterator->first)) {
                ...
                ++iterator;set<ptr<partial_sha3_hash>, Comparator> committedTransactions;
...
ptr<Transaction> PendingTransactionsAgent::getKnownTransactionByPartialHash(ptr<partial_sha3_hash> hash) {
    lock_guard<recursive_mutex> lock(transactionsMutex);
...
bool PendingTransactionsAgent::isCommitted(ptr<partial_sha3_hash> _hash) {
    lock_guard<recursive_mutex> lock(transactionsMutex);
...
void PendingTransactionsAgent::pushCommittedTransaction(shared_ptr<Transaction> t) {
    ...
    while (committedTransactions.size() > getNode()->getCommittedTransactionsHistory()) {
    ...
    getNode()->getCommittedTransactionsDB()->Put(writeOptions, Slice((const char *) t->getPartialHash()->data(), PARTIAL_SHA3_HASHLEN),
                                  Slice((const char*) &committedTransactionCounter, sizeof(committedTransactionCounter)));

    getNode()->getCommittedTransactionsDB()->Put(writeOptions, Slice("transactions"),
                                  Slice(to_string(committedTransactionCounter)));
